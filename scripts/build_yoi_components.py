...
# EDUCATION ACCESS: K-12 SCHOOLS + COLLEGES / UNIVERSITIES
# Uses:
# data/processed/overlays/school_locations.geojson
# data/processed/overlays/college_locations.geojson

school_geojson = DATA_DIR / "processed" / "overlays" / "school_locations.geojson"
college_geojson = DATA_DIR / "processed" / "overlays" / "college_locations.geojson"


def k12_school_distance_weight(distance_m):
    """
    K-12 school access distance weighting.
...
def college_distance_weight(distance_m):
    """
    College/postsecondary access distance weighting.
...
def calculate_point_access(points_path, access_col, count_col, weight_func):
    """
    Calculates distance-weighted access from tract centroids to point locations.
    """
    tract_gdf = anchor_gdf[["tract_geoid", "geometry"]].copy()
    ...
    if not points_path.exists():
        print(f"WARNING: missing point layer for {access_col}: {points_path}")

        for geoid in tract_gdf["tract_geoid"]:
            rows.append({
                "tract_geoid": geoid,
                count_col: 0,
                access_col: 0.0,
    ...
    if len(points) == 0:
        print(f"WARNING: point layer has zero rows for {access_col}: {points_path}")
    ...
    tract_proj = tract_gdf.to_crs(3310).copy()
    points_proj = points.to_crs(3310).copy()

    tract_proj["centroid"] = tract_proj.geometry.centroid

    for _, tract_row in tract_proj.iterrows():
        tract_geoid = tract_row["tract_geoid"]
        centroid = tract_row["centroid"]

        distances_m = points_proj.geometry.distance(centroid)
        weights = distances_m.apply(weight_func)

        nearby = weights > 0

        rows.append({
            "tract_geoid": tract_geoid,
            count_col: int(nearby.sum()),
            access_col: float(weights.sum()),
        })

    return pd.DataFrame(rows)


# Calculate education access indicators.
school_access_df = calculate_point_access(
    school_geojson,
    access_col="school_access",
    count_col="school_count_nearby",
    weight_func=k12_school_distance_weight,
)

college_access_df = calculate_point_access(
    college_geojson,
    access_col="college_university_access",
    count_col="college_university_count_nearby",
    weight_func=college_distance_weight,
)

# Merge into tract table.
for c in [
    "school_access",
    "school_count_nearby",
    "college_university_access",
    "college_university_count_nearby",
]:
    tracts = tracts.drop(columns=[c], errors="ignore")

tracts = tracts.merge(school_access_df, on="tract_geoid", how="left")
tracts = tracts.merge(college_access_df, on="tract_geoid", how="left")
...
for c in [
    ...
    tracts[c] = pd.to_numeric(tracts[c], errors="coerce").fillna(0.0)
...
add_indicator(
    ...
    "school_access",
    "education",
    tracts["school_access"],
    True,
    "CDE California Public Schools / School Sites 2024-25",
    "Distance-weighted access to K-12 school locations near each census tract."
)
...
add_indicator(
    ...
    "college_university_access",
    "education",
    tracts["college_university_access"],
    True,
    "NCES EDGE / IPEDS Postsecondary School Locations 2024-25",
    "Distance-weighted access to colleges, universities, and postsecondary institutions near each census tract."
)
...
# Uses data/processed/overlays/service_locations.geojson generated by
# scripts/build_service_locations_geojson.py

service_geojson = DATA_DIR / "processed" / "overlays" / "service_locations.geojson"


def distance_weight_miles(distance_m):
    """
    Convert distance in meters to an accessibility weight.
...
def classify_services_if_needed(svc_gdf: gpd.GeoDataFrame) -> gpd.GeoDataFrame:
    """
    Adds tier and indicator fields if they are not already present
    in service_locations.geojson.
    """
    svc_gdf = svc_gdf.copy()
    ...
    for c in [
        ...
        if c in svc_gdf.columns:
            text_parts.append(svc_gdf[c].fillna("").astype(str).str.lower())
    ...
    if text_parts:
        ...
        text = pd.Series("", index=svc_gdf.index)

    excluded = text.str.contains(
        r"not youth supports|not core youth supports|administrative|not a service site|move to health opportunity|move to health",
        regex=True,
        na=False,
    )
    ...
    development = text.str.contains(
        r"youth development|enrichment|mentoring|leadership|after[- ]?school|teen programming|life skills|positive youth|tutoring|arts|sports|college prep",
        regex=True,
        na=False,
    )

    barrier = text.str.contains(
        r"barrier|case management|transportation|childcare|benefits|legal aid|legal|accommodation|family support|foster|immigrant|refugee|disabil|navigation|health services|referrals",
        regex=True,
        na=False,
    )
    ...
    if "indicator_basic_navigation" not in svc_gdf.columns:
        svc_gdf["indicator_basic_navigation"] = basic.astype(int)

    if "indicator_youth_development" not in svc_gdf.columns:
        svc_gdf["indicator_youth_development"] = development.astype(int)

    if "indicator_barrier_reduction" not in svc_gdf.columns:
        svc_gdf["indicator_barrier_reduction"] = barrier.astype(int)

    if "indicator_transition_pathways" not in svc_gdf.columns:
        svc_gdf["indicator_transition_pathways"] = transition.astype(int)

    if "indicator_segment_specific" not in svc_gdf.columns:
        svc_gdf["indicator_segment_specific"] = segment.astype(int)

    if "count_in_youth_supports" not in svc_gdf.columns:
        svc_gdf["count_in_youth_supports"] = (
            ~excluded &
            (
                basic |
                development |
    ...
    if "tier_num" not in svc_gdf.columns:
        svc_gdf["tier_num"] = 0
        svc_gdf.loc[basic, "tier_num"] = svc_gdf.loc[basic, "tier_num"].clip(lower=1)
        svc_gdf.loc[development, "tier_num"] = svc_gdf.loc[development, "tier_num"].clip(lower=2)
        svc_gdf.loc[barrier, "tier_num"] = svc_gdf.loc[barrier, "tier_num"].clip(lower=3)
        svc_gdf.loc[transition, "tier_num"] = svc_gdf.loc[transition, "tier_num"].clip(lower=4)
        svc_gdf.loc[svc_gdf["count_in_youth_supports"] == 0, "tier_num"] = 0

    svc_gdf["tier_num"] = pd.to_numeric(svc_gdf["tier_num"], errors="coerce").fillna(0).astype(int)

    if "tier_weight" not in svc_gdf.columns:
        tier_weights = {
            0: 0.0,
            1: 1.0,
            2: 1.5,
        ...
        svc_gdf["tier_weight"] = svc_gdf["tier_num"].map(tier_weights).fillna(0.0)
    else:
        svc_gdf["tier_weight"] = pd.to_numeric(svc_gdf["tier_weight"], errors="coerce").fillna(0.0)

    return svc_gdf


# Default empty columns in case the service layer is missing.
for c in [
    "service_count",
    "tier_weighted_youth_program_access",
    "youth_development_access",
    "barrier_reduction_access",
    "transition_pathway_access",
    "segment_specific_support_access",
]:
    tracts[c] = 0.0


if service_geojson.exists():
    print("Using youth service locations:", service_geojson)

    svc_gdf = gpd.read_file(service_geojson)

    if svc_gdf.crs is None:
        svc_gdf = svc_gdf.set_crs("EPSG:4326")

    svc_gdf = svc_gdf[svc_gdf.geometry.notna()].copy()
    svc_gdf = classify_services_if_needed(svc_gdf)

    # Keep only rows that count toward Youth Supports.
    svc_gdf["count_in_youth_supports"] = pd.to_numeric(
        svc_gdf["count_in_youth_supports"],
        errors="coerce"
    ).fillna(0).astype(int)

    svc_youth = svc_gdf[svc_gdf["count_in_youth_supports"] == 1].copy()

    print("Youth Support service points used for scoring:", len(svc_youth))

    if len(svc_youth) > 0:
        # Use tract polygons from the already-loaded boundary GeoJSON.
        tract_gdf = anchor_gdf[["tract_geoid", "geometry"]].copy()
        ...
        # Project to California Albers for distance calculations in meters.
        tract_proj = tract_gdf.to_crs(3310).copy()
        svc_proj = svc_youth.to_crs(3310).copy()

        # Use tract centroids as the demand point.
        tract_proj["centroid"] = tract_proj.geometry.centroid

        # Ensure numeric tier/indicator fields.
        svc_proj["tier_weight"] = pd.to_numeric(
            svc_proj["tier_weight"],
            errors="coerce"
        ).fillna(0.0)
        ...
        for c in indicator_cols:
            if c not in svc_proj.columns:
                svc_proj[c] = 0
            svc_proj[c] = pd.to_numeric(svc_proj[c], errors="coerce").fillna(0).astype(int)
        ...
        for _, tract_row in tract_proj.iterrows():
            ...
            distances_m = svc_proj.geometry.distance(centroid)
            distance_weights = distances_m.apply(distance_weight_miles)

            within_access = distance_weights > 0

            if within_access.sum() == 0:
                access_rows.append({
                    "tract_geoid": tract_geoid,
                    "service_count": 0,
                    "tier_weighted_youth_program_access": 0.0,
                    "basic_navigation_access": 0.0,
                    "youth_development_access": 0.0,
                    "barrier_reduction_access": 0.0,
                    "transition_pathway_access": 0.0,
                    "segment_specific_support_access": 0.0,
                })
                continue

            nearby = svc_proj.loc[within_access].copy()
            w = distance_weights.loc[within_access].astype(float)
            tier_w = nearby["tier_weight"].astype(float)

            base_weight = tier_w * w

            access_rows.append({
                "tract_geoid": tract_geoid,
                "service_count": int(within_access.sum()),

                # Overall youth program access, weighted by tier and distance.
                "tier_weighted_youth_program_access": float(base_weight.sum()),

                # Indicator-specific access.
                "basic_navigation_access": float((base_weight * nearby["indicator_basic_navigation"]).sum()),
                "youth_development_access": float((base_weight * nearby["indicator_youth_development"]).sum()),
                "barrier_reduction_access": float((base_weight * nearby["indicator_barrier_reduction"]).sum()),
                "transition_pathway_access": float((base_weight * nearby["indicator_transition_pathways"]).sum()),
                "segment_specific_support_access": float((base_weight * nearby["indicator_segment_specific"]).sum()),
            })

        access_df = pd.DataFrame(access_rows)

        # Drop default placeholder columns before merge.
        drop_cols = [
            "service_count",
            "tier_weighted_youth_program_access",
            "basic_navigation_access",
            "youth_development_access",
            "barrier_reduction_access",
            "transition_pathway_access",
            "segment_specific_support_access",
        ...
        tracts = tracts.drop(columns=[c for c in drop_cols if c in tracts.columns], errors="ignore")
        tracts = tracts.merge(access_df, on="tract_geoid", how="left")
    ...
    print("WARNING: service_locations.geojson not found. Youth Supports indicators will be zero.")
    print("Expected:", service_geojson)
...
add_indicator(
    ...
    "tier_weighted_youth_program_access",
    "youth_supports",
    tracts["tier_weighted_youth_program_access"],
    True,
    "service_locations.geojson",
    "Tier-weighted and distance-weighted access to youth service programs within 10 miles."
)
...
add_indicator(
    ...
    "youth_development_access",
    "youth_supports",
    tracts["youth_development_access"],
    True,
    "service_locations.geojson",
    "Mentoring, leadership, after-school, teen programming, enrichment, and positive youth development access."
)
...
add_indicator(
    ...
    "barrier_reduction_access",
    "youth_supports",
    tracts["barrier_reduction_access"],
    True,
    "service_locations.geojson",
    "Case management, transportation, childcare, benefits navigation, legal aid, family support, and other barrier-reduction access."
)
...
add_indicator(
    ...
    "transition_pathway_access",
    "youth_supports",
    tracts["transition_pathway_access"],
    True,
    "service_locations.geojson",
    "Workforce training, paid work experience, GED/dropout recovery, career, job coaching, and earn-and-learn pathway access."
)
...
add_indicator(
    ...
    "segment_specific_support_access",
    "youth_supports",
    tracts["segment_specific_support_access"],
    True,
    "service_locations.geojson",
    "Support access for foster youth, out-of-school youth, parenting youth, disabled youth, LGBTQ+ youth, immigrant/refugee youth, homeless youth, and justice-involved youth."
)
